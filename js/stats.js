...
function loadGeneralOverview() {
    ...
    if (!mainAnalysis) {
        console.error('mainAnalysis element not found');
        return;
    }
    ...
    // Add event listeners for the ranking dropdowns with proper null checks
    setTimeout(() => {
...
function showModalChart(games, chartType, chartTitle) {
    ...
    if (!modal || !modalTitle || !modalChart || !modalBody || !modalContainer) {
...
function initializeWhenReady() {
    // Wait for both DOM and all required dependencies
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initializeWhenReady);
        return;
    }
    
    // Check if required dependencies are loaded with retries
    if (typeof Chart === 'undefined' || typeof bootstrap === 'undefined') {
        console.warn('Dependencies not ready, retrying in 100ms...');
    ...
    if (!mainContainer) {
        console.warn('Main container not ready, retrying in 100ms...');
        setTimeout(initializeWhenReady, 100);
        return;
    }
